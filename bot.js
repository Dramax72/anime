...
const TOKEN = '7435709953:AAEQOPjjRiS24rV7oLr45uggdMQWl4tpfCg';
const MOVIE_CHANNEL_ID = '--1002368973765;
const BOT_CHANNEL_ID = '-1002283829065';
const SUBSCRIPTION_CHANNELS = [
  { id: '-1002125763301', username: '@Anifan_dub' }
];

const CREATOR_ID = 6571202128;
const ADMIN_IDS = [5633256442, 6805505142];
  ...
  if (isSubscribed) {
    ...
    if (code && postMap[code]) {
      ...
      try {
        ...
        const responseMessage = `Raqamga mos keladigan Anime: ${code}`;
        bot.sendMessage(chatId, responseMessage);
        saveUserChatHistory(chatId, responseMessage, true);
      } catch (error) {
        console.error(error);
        const errorMessage = 'Anime yuborishda xatolik yuz berdi.';
        bot.sendMessage(chatId, errorMessage);
        saveUserChatHistory(chatId, errorMessage, true);
      }
    } else {
      const welcomeMessage = `Assalomu aleykum! Menga kod yuboring va men siz uchun uning Animesini topib beraman. Agar kodlarni bilmaysiz, ushbu kanaldan topishingiz mumkin: https://t.me/Anifan_dub`;
      ...
      const userButtons = [
        ...
        [{ text: 'Anime tavfsiya qilish (New)', callback_data: 'suggest_movie' }] // New button
      ];
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  if (data === 'suggest_movie') {
    const initialMessages = [
      "Assalomu aleykum bu panelda siz botga yangi Anime qo'shish tavfsiyasini berishingiz mumkin",
      "Botga qo'shmoqchi bo'lgan Animeni nomi, sanasi va janrlarini qo'shib yozib tashlang va admin bu Animeni tasdiqlasa botga va kanalga sizning Animeyingiz qo'shiladi"
    ];
...
bot.onText(/^\/code (\d+)$/, async (msg, match) => {
  ...
  if (postMap[code]) {
    ...
    const errorMessage = 'Bunday Anime mavjud emas';
    bot.sendMessage(chatId, errorMessage);
    saveUserChatHistory(chatId, errorMessage, true);
  }
});
...
bot.on('message', async (msg) => {
  ...
  if (pendingSendMessages[userId]) {
    ...
    try {
      ...
      const forwardedMessageId = forwardedMessage.message_id;
      await bot.copyMessage(chatId, BOT_CHANNEL_ID, forwardedMessageId);
      const responseMessage = `Raqamga mos keladigan Anime: ${userInput}`;
      bot.sendMessage(chatId, responseMessage);
      saveUserChatHistory(chatId, responseMessage, true);
    } catch (error) {
      console.error(error);
